...
static inline void vga_text_set(size_t x, size_t y, unsigned char c)
{
    ...
    for (size_t j = 0; j < KFONT_VGA_HEIGHT; j++) {
        for (size_t i = 0; i < GLYPH_WIDTH; i++) {
            ...
            size_t fb_off = term->fb_scanline * (KFONT_VGA_HEIGHT * y + j) +
                            (GLYPH_WIDTH * x + i) * 4;
...
void terminal_init(size_t screen_width, size_t screen_height,
                   uint8_t *framebuffer, size_t fb_scanline)
{
    term->row = 0;
    term->col = 0;
